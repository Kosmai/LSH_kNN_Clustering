...
unsigned int HashFunctionG::computeG(std::vector<double>& p){
	long term;
	unsigned int termMod;
	unsigned int total = 0;
	const unsigned int M = 0xffffffff - 4; //prime number 2^32 - 5

	//linear combination of all h(p) funcions
...
unsigned int HashFunctionG::computeID(std::vector<double>& p, unsigned int& tableSize){
	return this->computeG(p) % tableSize;
}
